    ...
    return (
        <div className="glass rounded-3xl p-5 space-y-4">
            <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-slate-800">{t('student.activity.title')}</h3>
                {currentLearner.isInQuiz && (
                    <span className="ml-auto flex items-center gap-1 bg-amber-100 border border-amber-200 text-amber-700 text-xs font-medium px-2 py-0.5 rounded-lg">
                        <Shield className="w-3 h-3" /> {t('student.activity.quizStatus')}
                    </span>
                )}
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
                <div className="text-center">
                    ...
                    <p className="text-xs text-slate-500">{t('common.date').includes('Date') ? t('student.performance.time') : 'Session'}</p>
                </div>
                <div className="text-center">
                    <div className="flex items-center justify-center gap-1 mb-1">
                    ...
                    <p className="text-xs text-slate-500">{t('student.performance.retries')}</p>
                </div>
                <div className="text-center">
                    <div className="flex items-center justify-center gap-1 mb-1">
                    ...
                    <p className="text-xs text-slate-500">{t('instructor.kpis.interventions').split(' ')[0]}</p>
                </div>
            </div>
        </div>
    );
